...
export function createLogger(component: string) {
  const log = (level: LogLevel, event: string, data: Record<string, unknown> = {}, windowId?: string, error?: string) => {
  ...
  return {
    debug: (event: string, data?: Record<string, unknown>, windowId?: string) => {
      log('DEBUG', event, data, windowId);
    },
    info: (event: string, data?: Record<string, unknown>, windowId?: string) => {
      log('INFO', event, data, windowId);
    },
    warn: (event: string, data?: Record<string, unknown>, windowId?: string, error?: string) => {
      log('WARN', event, data, windowId, error);
    },
    error: (event: string, data?: Record<string, unknown>, windowId?: string, error?: string) => {
      log('ERROR', event, data, windowId, error);
    },
  };
}

/**
 * Default logger for electron-main component
 */
export const logger = createLogger('electron-main');
